...
**Created:** 2026-05-06
**Last Updated:** 2026-05-06 (Milestone 1 build started)
**Version:** v0.2 (Week 1 in progress)
...
| Storage | **Local disk now → AWS S3 from M6+** | Don't burn credit on dev-phase storage |
| Compute | **Local Mac (dev) → AWS EC2 (M7+ if 24/7 needed)** | M-series Macs are faster than t3.medium for video work; iterate locally first |
...
- ✅ Project skeleton scaffolded (folders, configs, env handling)
- ✅ Telegram bot accepts YouTube links (`/start` + message handler)
- ✅ `worker/download.py` — yt-dlp wrapper, extracts audio as 16kHz mono 64kbps MP3 (Whisper-optimized)
- ✅ `worker/transcribe.py` — Groq Whisper integration with word-level timestamps + automatic chunking for files >24MB
- ✅ End-to-end: send YT link → get transcript file back via Telegram
- ❌ Clip selection (Gemini) — NOT yet
- ❌ Video cutting / subtitles — NOT yet
- ❌ Speaker-aware cropping — NOT yet
- ❌ S3 upload — NOT yet
- ❌ Posting (manual for now)

---

## 5. What Is In Progress

**Milestone 1 delivered.** User now needs to:
1. Create a Telegram bot via BotFather and get the token
2. Set up local Python env + install FFmpeg system binary
3. Add keys to `.env` and run `python main.py`
4. Test on a short (5 min) YT video first, then a real podcast

**Next milestone (M2):** Gemini clip selection — feed transcript JSON, get back 40–60 ranked viral clip ranges with hooks and captions.
...
| Where does worker run during dev? | Local Mac vs EC2 | **RESOLVED:** Local Mac through M5, AWS S3 from M6, AWS EC2 only if/when auto-posting is built |
...
| Which caption styles to support in MVP? | All 5 (highlighted/scale/minimalist/box/none) vs subset | Start with `highlighted` + `minimalist` (proven defaults from Vugola), add others if requested |
...
- **Vugola (paid competitor — reference only):** Does exactly what we're building. Pricing: $9/$29/$99 per month. Their feature set validates ours: aspect ratios `9:16`/`1:1`/`16:9`, caption styles `highlighted`/`scale`/`minimalist`/`box`/`none`, virality scoring, multi-platform posting. Investigate their multi-platform posting backend (likely Ayrshare or similar aggregator) when we hit M7.
